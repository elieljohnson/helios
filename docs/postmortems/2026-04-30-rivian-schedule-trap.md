...
- [ ] **[future-deferred] v6 Rivian-via-local-BLE-daemon** — now the only remaining technical path for cloud-controlled stops on this car. Architecture: small always-on daemon on the user's home network (Pi/NUC/spare Mac), BLE-paired with the car at first run, exposing an HTTP endpoint Helios cron POSTs to. Daemon signs commands using the BLE-paired keypair and forwards via BLE. The crypto helpers in `app/src/lib/rivian/crypto.ts` are reusable; the cloud-side `EnrollPhone` flow + meta storage in `oauth_tokens` are reusable. Substantial new surface area (probably 20–40h end-to-end including BLE protocol work, network plumbing from Vercel to home, daemon hardening, install UX). Pursue or defer is a strategic decision, not a tactical one.
