...
- [x] **Wire proper Rivian `STOP_CHARGING` via vehicle-command API** *(2026-05-01 session)*. Replaces the v4 no-op with a one-shot HMAC-signed command. Surface verified against `bretterer/rivian-python-client`; the actual command names are `STOP_CHARGING` / `START_CHARGING` / `CHARGING_LIMITS` (not `CHARGE_STOP` as the postmortem originally guessed). Implementation: ECDH(ourPrivate, vehiclePublic) → HKDF-SHA256 → HMAC-SHA256 over `command || timestamp`, all via Node's built-in `crypto` (no new deps). Effort came in at ~6h end-to-end vs. the 2–4h estimate, mostly because the postmortem under-counted the phone-key enrollment flow and HMAC plumbing. Belt-and-suspenders `setChargeLimit` (CHARGING_LIMITS with `SOC_limit` param) also wired. `startCharging` deliberately left on the schedule API for now — that path currently works and migrating both surfaces simultaneously would stack risk on the only working actuator. Commits: crypto helpers, enrollment flow, sendVehicleCommand wrapper + v5 stop, verification loop.
- [x] **Add post-stop verification loop** in cron route *(2026-05-01 session)*. Pure function in `lib/verifyEvAction.ts` reads the recent actions, finds the most recent successful stop, and logs a charge-action with `ok: false` and reason "stop ack'd Ns ago but car still drawing N kW" if `ev_w > 100 W` within the 60–600 second window. Voids itself when an intervening start fires (engine flipped its mind) or a verification-failure entry already exists for this stop (no duplicate logs). 11 unit tests pin the state machine. Same skeleton can later cover `setBackupReserve` and `startCharging` (still open as the next bullet).
