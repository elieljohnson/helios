...
- [ ] **Wire proper Rivian `CHARGE_STOP` via vehicle-command API** (~2–4h; replaces the no-op). Promoted to P0 — the system has no working stop until this lands. While in the command API, also probe whether `setChargeLimit` (or equivalent) is available so a "true stop" can be a belt-and-suspenders flow: (a) send `CHARGE_STOP`, (b) lower the *profile-level* charge limit to current SoC, (c) verify `ev_w → 0` on the next tick. Each Rivian state level (session vs. profile) has its own auto-revert pathway, demonstrated by the 2026-05-01 morning observation that a user-set session limit reverted overnight to the profile default while a manual Stop press at the contactor level held.
