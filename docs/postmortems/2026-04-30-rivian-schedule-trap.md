...
- [~] **Wire proper Rivian `STOP_CHARGING` via vehicle-command API** *(code written 2026-05-01; awaiting live verification)*. Five commits sit local-unpushed pending a live smoke test against the actual car. Surface verified against `bretterer/rivian-python-client`; the real command names are `STOP_CHARGING` / `START_CHARGING` / `CHARGING_LIMITS` (not `CHARGE_STOP` as the postmortem originally guessed). Implementation: ECDH(ourPrivate, vehiclePublic) → HKDF-SHA256 → HMAC-SHA256 over `command || timestamp`, all via Node's built-in `crypto` (no new deps). Belt-and-suspenders `setChargeLimit` (CHARGING_LIMITS with `SOC_limit` param) also wired. `startCharging` deliberately left on the schedule API for now — that path currently works and migrating both surfaces simultaneously would stack risk on the only working actuator. **Open risk**: `bretterer`'s docstring notes phone keys "also need to be paired locally via BLE." We're betting cloud-only commands work without BLE pairing for charging actions specifically (multiple community projects suggest this is true), but it's not confirmed. Live test will tell. Effort came in at ~3.5h coding + tests vs. the 2–4h postmortem estimate, before any live debugging.
- [~] **Add post-stop verification loop** in cron route *(code written 2026-05-01; ships with the v5 stop)*. Pure function in `lib/verifyEvAction.ts` reads the recent actions, finds the most recent successful stop, and logs a charge-action with `ok: false` and reason "stop ack'd Ns ago but car still drawing N kW" if `ev_w > 100 W` within the 60–600 second window. Voids itself when an intervening start fires (engine flipped its mind) or a verification-failure entry already exists for this stop (no duplicate logs). 11 unit tests pin the state machine. Generalizable skeleton — same shape can later cover `setBackupReserve` and `startCharging` (still open as the next bullet).
