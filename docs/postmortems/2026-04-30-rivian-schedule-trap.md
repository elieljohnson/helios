...
- [ ] **[New 2026-05-01] Smartcar V3 actuator migration is now P0** — only path to working stop authority for our deployment shape. Plan in `docs/smartcar-integration-handoff.md`. Doable without the car (V3 commands docs + code), live test needs the car.
- [ ] **[New 2026-05-01, future-deferred] v6 Rivian-via-local-BLE-daemon** — only pursue if Smartcar V3 actuators also fail. Architecture: small always-on daemon on the user's home network (Pi/NUC), BLE-paired with the car at first run, exposing an HTTP endpoint Helios cron POSTs to. Daemon signs commands using the BLE-paired keypair and forwards via BLE. The crypto helpers in `app/src/lib/rivian/crypto.ts` are reusable for this path; the cloud-side `EnrollPhone` flow + meta storage in `oauth_tokens` are reusable too.
