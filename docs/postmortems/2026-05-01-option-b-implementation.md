...
| `26412ab` → `0411dc8` → `9de1af0` | fix (system spec) | Three iterations to land on the right system numbers after pulling the stamped install plans. Final: 16.1 kW DC / 14.82 kW CEC-AC / 13.3 kW AC inverter ceiling (35 × IQ8X-80 @ 380 VA), 40.5 kWh Powerwall 3 storage, full-house backup config (2× Backup Gateway 3), 21,660 kWh year-one estimate at 154% offset. mock.ts + three case-study docs + persistent memory all updated to match. |
| `5c780df` | fix (forecast) | `SYSTEM_PEAK_KW` constant in `weather.ts` was hardcoded at 9.5 — legacy from when system was thought smaller. Forecast was scaling Open-Meteo irradiance to ~71% of reality, feeding too-low `solarRemaining` values into `decideEvCharge` and biasing toward premature stops. Bumped to 13.3 (matches AC clip ceiling). mock.ts solarCurve constant updated to match. |
| `9535f7e` | fix (enphase) | `consumption_meter` URL embedded a moving timestamp that changed every second; Next.js fetch cache keys on URL, so every call generated a unique URL and bypassed the cache entirely. Surfaced by an Enphase rate-limit-violation email (1,662 hits this month, 1,000/month free quota). Fixed by rounding `start_at` to a 15-min bucket. Also bumped cache TTL 5 → 15 min and corrected misleading "1000/day" comment to the actual "1000/month" limit. |
| `b9f71b2` | fix (integrations) | `/api/integrations` was doing a live `getSummary` ping on every poll → 60 calls/hour to Enphase whenever Settings was open in any tab. The actual quota drain (the dashboard's `assembleStatus` doesn't call Enphase — Tesla owns solar there). Replaced with token-existence check. Enphase row stays green when connected; the live "X.XX kW now" number is gone from this row but live solar production already shows on the dashboard SOLAR card. Drops month-to-month Enphase usage from thousands to single digits. |

**Counts:** 24 commits. Build green at every checkpoint. 89/89 tests pass at session end.
...
8. **`SYSTEM_PEAK_KW` constant was a stale legacy value, biasing every forecast.** `weather.ts` had `SYSTEM_PEAK_KW = 9.5` hardcoded — set once at the start of the project when the system was thought smaller, never reconciled to reality. The actual AC inverter ceiling is 13.3 kW. Open-Meteo's irradiance was being scaled to ~71% of true production. That fed straight into `decideEvCharge.budget`: every "we don't have enough remaining solar" stop was running on a ~30% underestimate of the day's remaining production. Surfaced only when the user asked "did you check the Enphase and Tesla specs?" after observing 13 kW peaks the codebase claimed shouldn't be possible. **Lesson:** any project-wide constant that encodes a piece of physical reality (panel count, peak kW, battery capacity, etc.) should be reconciled to the actual install spec EARLY. The cost of getting it wrong compounds across every consumer. Worth a one-time "verify against installer paperwork" gate during onboarding.

9. **Enphase API quota was being burned by a cache-busting URL bug AND a per-poll live ping.** The Watt plan is 1,000/month (not 1,000/day, as a code comment claimed). Two compounding causes: (a) `getConsumptionPower` embedded `start_at = Math.floor(Date.now()/1000) - 7200` in the URL, ticking every second — Next.js's fetch cache keys on URL, so every call generated a new URL and the `revalidate: 300` setting was completely defeated. (b) `/api/integrations` did a live `getSummary` ping on every 60-second poll, and the dashboard's IntegrationsCard kept that endpoint hot whenever Settings was open. Combined: 1,662 hits/month observed pre-fix vs. ~1,000/month quota. **Fix:** round timestamp parameters to a stable bucket so repeated calls hit the cache; bump cache TTL to match upstream telemetry update cadence (15 min for Envoy-S); replace the per-poll live ping in `/api/integrations` with a token-existence check. The dashboard's SOLAR card already shows live solar production via Tesla, so the live number is reachable elsewhere. **Lesson:** any external API call where the URL contains `Date.now()` is a cache footgun. Round to a bucket. And know your provider's quota cadence before sizing the cache TTL.
